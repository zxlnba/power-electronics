...
> | 60V 低压测试 | ⚠️ **发现设计点不匹配**：现腔 Z0=181.5Ω 按额定 Q=1.4 设计，60V 可用负载换算 Q=2~8，高 Q 无升压区 + 死区/二极管 ~20% 损耗 → 增益封顶 M≈0.8，60V 每路只能到 ~24-27V |
> | 改腔方案 | 🔧 待执行：Z0=52.2Ω（Lr 79µH / Cr 29nF）可解锁 60V 下 78-200W 功率阶梯 |
> | 额定 400V 测试 | ⏳ 需 360-440V 母线 + 160Ω/80Ω 负载（见 `docs/工程开发文档.md` §4） |
>
> **完整工程开发记录（含 60V 缩尺测试教训、损耗根因、改腔设计）见
...
│   ├── llc_rescale.m                            ← 60V 改腔设计脚本（Z0=52.2）
│   ├── llc_sim.m                                ← 时域仿真 + 损耗灵敏度（复现实测）
│   ├── scaled-low-voltage-test-plan.md          ← 缩尺测试方案（⚠️ §3 已作废，见顶部更正）
