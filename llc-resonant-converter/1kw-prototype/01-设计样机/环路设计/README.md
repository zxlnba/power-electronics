...
| 励磁电感 | Lm | **2.0mH（设计书 §3.2 k=7.4 取值，LCR 实测一致）** | λ=Lr/Lm≈0.135（m=7.4） |
...
fm = 1/(2π·√((Lr+Lm)·Cr)) = 36.9 kHz  第二谐振频率（Lm 参与）
Z0 = √(Lr/Cr) = 181.5 Ω              特征阻抗
Rac = (8n²/π²)·Rdc = 518.5 Ω        反射交流等效电阻（设计书 §3.2，n=2，Rdc=160Ω）
Q  = Z0/Rac = 0.35                   满载品质因数（设计书取值，符合 0.3-0.5）
```

> Q 口径说明：本设计采用**参数作品设计书 §3.2** 口径 `Q=0.35`（`Rac=(8n²/π²)·Rdc`，
> n=2 每路变换比，Rdc=400V²/1kW=160Ω）。若按功率守恒折算 `Rac'=V1rms²/P=129.7Ω`，
> 等效 Q≈1.4（=0.35×4，差 n²）。**K_f 与 2p2z 系数只依赖 m 与输出极点 fp_out，与 Q
> 无关**，故两种口径下固件系数完全相同。
...
|---|---|---|---|
| 满载 | 0.35 | **1.034**（受 fs_min 限制，峰值 @fn=0.888） | 387V |
| 50% | 0.175 | 1.037（受 fs_min 限制，理论峰够不到） | 386V |
| 25% | 0.0875 | 1.037（受 fs_min 限制，理论峰够不到） | 386V |

> **⚠️ 设计发现：满载升压裕量不足。**
> 360V 输入需要 M = 400/360 = **1.111**，但满载下谐振腔在频带内最多提供 1.034。
> 意味着**满载、低输入时输出将跌落，无法维持 ±200V**；此时控制器会一路压到
> fs_min 仍不够（fn=0.888 已是频带下限）。应对措施：
> 1. 若必须 360V 满载稳压，需**提高 M_max**：增大 Lm/Lr 比值（m↑ → M_max↑）
>    或放宽 fs_min（fs_min 越低 fn 越小增益越高，但 ZVS/死区裕量变差）。
> 2. 控制上：把 fs_min 设到 95kHz，接受低压满载时的输出电压随输入跌落
>    （限功率而非稳压）。轻载受 fs_min 限制更早，约 386V 起才会不足。
...
```
K_f(fs=fr) = (Vin/n) · dM/dfn|fn=1 / fr = (Vin/n) · (-2/m) / fr = -0.505 V/kHz
```

- **与负载无关**（负载只改变输出极点位置，见 §5）——这是早期版本用错误 FHA
  公式得到的"4× 变化"结论的更正，轻载环路实际比满载更稳（§6 裕度表）。
- `m = Lm/Lr`：K_f ∝ 1/m。设计书 §3.2 取 k=7.4（Lm=2.0mH）→ K_f=-0.505 V/kHz；
  若改绕到 m=4，K_f 会升到约 -0.93 V/kHz。
...
| 环节 | 频率 | 作用 |
|---|---|---|
| ωz_lm = 2π·fm | 36.9kHz | Lm 谐振零点，远高于穿越频率，可忽略 |
...
C(s) = K_c · (1 + s/ωz) / (s·(1 + s/ωp))
      ...
      K_c = -2.4922e7   ← 使 |C·Gvd|@fco = 1（负号编码反相：fs↑→Vout↓）
```

| 指标 | 连续域 | 离散（含 1 拍延迟） |
|---|---|---|
| 穿越频率 fco | 1960 Hz | 1963 Hz |
| 相位裕度 PM | 80.9° | **71.0°** |
| 增益裕度 GM | 43.3 dB | — |

负载敏感性（K_f 恒定，fp_out 左移；Q 按设计书口径 0.35 满载）：

| 负载 | Q | fp_out | fco | PM | GM |
|---|---|---|---|---|---|
| 满载 | 0.35 | 19.9 Hz | 1960 Hz | 80.9° | 43.3 dB |
| 50% | 0.175 | 9.95 Hz | 993 Hz | 84.8° | 49.3 dB |
| 25% | 0.0875 | 4.97 Hz | 498 Hz | 85.9° | 55.3 dB |
...
> **抗饱和**：撞钳位（fs 越界）时**冻结内部状态 x1/x2**（不更新），仅钳位 fs——频率被
> 限在 95–130kHz 时状态不再累积，防积分饱和后恢复时频率回冲。固件实现见
> `../../03-调试代码/src/llc_ctrl.c` 的 `llc_ctrl_update()`（与 60V 闭环固件
> "撞钳位冻结积分"同基线）。
...
> 偏离精确 AC 电路传递），K_f 低估 40%（-0.535 → -0.748 V/kHz）且误判为随负载
> 4× 变化。随后按设计书 §3.2 取值 m=7.4（Lm=2.0mH，LCR 实测一致，非早期脚本误设的
> m=5），K_f 修正为 -0.505 V/kHz，K_c 改为 -2.4922e7 保持 fco=1960Hz。本版已统一
> 为标准 FHA 公式 + 设计书 m，固件 2p2z 系数已同步更新。Q 采用设计书口径 0.35，
> M_max 满载 1.034（fn=0.888 频带内，最低输入 387V）。
...
- 最大电压偏差 ≈ **50mV**（0.025%），毫秒级恢复（扰动后首个开关周期即回落）
- 频率指令从 107kHz 抬到 ~117kHz（抵消 +10kHz 输入扰动，等效输出跌 -5.1V），
  未撞 130kHz 上限
...
4. **噪声检查**：fs 阶跃分辨率 67Hz → 输出分辨率约 34mV（K_f·Δfs = 0.505×67），
   验证不产生极限环。
...
| **采样时间** | 控制率上限 | ADC 采样 ≤ 4.5µs（已由 640 周期 60µs 缩短到 12.5 周期 ≈2.4µs，见 §9） |
...
> 60V 缩尺腔压降根因结论（副边增益健康、整流管 RHRP860 VF，换 MBR10100）见该文档 §5.3。
