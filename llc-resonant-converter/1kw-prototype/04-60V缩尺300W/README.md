...
**实测阈值（2026-08-21）**：啸叫在 **I_in ≈ 2.5A（≈141W）** 开始，对应 **I_tank ≈ 4.8A RMS**（√(2.3²+4.18²)）。**不变量是腔体电流、不是输入电流**——因 I_mag 恒流 4.18A，固定输入功率 ≈ 固定腔体电流，故与负载无关；40Ω 升压区（70kHz，I_mag ∝ 1/fs 更大）输入会在 2.5A 前提前到阈值（待实测确认）。2.5A 是**声学门槛而非危险线**，磁致伸缩良性，台架用途无需处理。最可疑声源 = 谐振电感 Lr（磁芯 B ∝ I_tank，随腔体电流走）。想安静：加大 Lm 降循环电流，或给 Lr 加机械阻尼（灌胶/夹紧）。
