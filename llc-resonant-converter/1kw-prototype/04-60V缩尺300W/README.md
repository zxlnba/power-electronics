...
> **⚠️ 2026-08-13 实际器件更新**：谐振腔已按实际到手器件落地 —— **Lr=8µH、Cr=470nF**
> （原方案此处为 Lr=7µH，fr 由 87.7kHz 变为 **82.1kHz**）。新变压器按实际器件绕制：
> **1:1、6T、Lm=26µH、气隙 0.32mm**，权威规格见
> **[`PQ3535变压器绕制设计.md`](./PQ3535变压器绕制设计.md)**（含时域仿真验证，
> 励磁比 k=3.2 经"1:1 稳压 + 绕制难易"核对确认）。下文 §3-§7 为原方案设计值，仅作原理参考。

> **🔬 2026-08-18 实测进展：压降根因已定位**：**fr≈75kHz**（增益=1 实测，设计 82.1k，差 6-8%
> 属寄生/容差正常，Lr/Cr 无异常）。整流管（RHRP860）全程只吃 **~2-3V**（两只串），
> **高频/低频的大掉压分别是增益曲线与容性硬开关区所致，不是整流管**——详见 §11。
...
## 11. 实测进展：压降根因定位（2026-08-18）

### 11.1 实测基准（用户直接测量，权威）

- 拓扑确认：**全桥**（4 管，原边 ±Vin 方波），Vout = M×Vin（1:1 变比）
- **fr ≈ 75kHz**（增益=1）：30V/75kHz 副边满摆 30V 方波；60V/80kHz 副边满摆 60V。
  设计 82.1kHz，差 6-8% 属寄生/容差正常，Lr/Cr 无异常（此前"真 fr≈55~60k"系拿系统增益
  对比无损下限的错判，作废）
- 60V 输入增益曲线实测：**80kHz → 副边 60V（M≈1）；100kHz → ~50V（M≈0.85）**

### 11.2 压降根因（多轮排查的结论）

"副边整流前波形健康、整流后掉 5-9V"按频段拆：

| 频段 | 副边（60V 输入） | 掉压来源 |
|---|---|---|
| 90-100kHz（>fr） | 自身只剩 ~50V（M≈0.85） | **增益曲线**，非整流管 |
| 60-70kHz（<fr） | M 本应 >1，输出却 45→51V 爬不上去 | **容性/硬开关区增益塌**，非整流管 |
| 75-80kHz（≈fr） | 满摆 60V | **整流管 2×VF ≈ 2-3V**，唯一背锅处 |

整流管 **RHRP860**（600V/8A 快恢复）：VF 随电流 **~1.0-1.5V/只**（0.5A→1.0V、2A→1.3V、
8A 额定 max 2.1V），两只串 **~2-3V**。**不是固定 2.1V**（2.1V 是 8A 额定 max）。二极管全程
只吃 ~2-3V，从不是大数——高频/低频那几次大掉压与它无关。

### 11.3 修复与验证

整流管换 **MBR10100**（10A/100V TO-220AC 肖特基，单管；勿买 CT 双管、勿买 60V 档）：

- VF ~0.5-0.6V/只，两只串 ~1.2V → 谐振点输出 **+2~3V**
- 无反向恢复 → 75-80kHz 省一块损耗，效率 **+3~5 点**
- 验证点：**60V / 80kHz / 40Ω**（M=1，副边满摆 60V）→ 预期 **~57-58V**（RHRP860 时 ~55-56V）

### 11.4 效率曲线是"山丘"

- 峰值在谐振点上方附近（~78-85kHz）
- 更高频低增益低功率、更低频丢 ZVS 硬开关 + 环流 → 两侧都降
- 60V/90kHz 的 75-80% 是**离峰工作点**，不是故障；之前"67%"是坏数据（2×20Ω 载不可能那么低）

### 11.5 输入电流读数约定

输入电流一律从**电源屏显**读（不用万用表 DC 档）；效率 = Vout²/R / (Iin×Vin)，Pin 会随
输出功率同步变化，不能拿同一个 Pin 硬算"补回压降后的效率"。
