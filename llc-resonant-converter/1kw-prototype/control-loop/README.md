...
|---|---|---|---|
| 满载 | 1.40 | **1.005**（峰值 @fn≈0.964，频带内可达） | 398V |
| 50% | 0.70 | 1.022（受 fs_min 限制，理论峰 @fn≈0.75 够不到） | 391V |
| 25% | 0.35 | 1.034（受 fs_min 限制，理论峰 @fn≈0.47 够不到） | 387V |

> **⚠️ 设计发现：满载升压裕量严重不足。**
> 360V 输入需要 M = 400/360 = **1.111**，但满载下谐振腔在频带内最多提供 1.005。
...
- `m = Lm/Lr` 必须实测：K_f ∝ 1/m。LCR 实测原边 Lm=2mH → m=7.4 → K_f=-0.505 V/kHz；
...
| 负载 | fp_out | fco | PM | GM |
|---|---|---|---|---|
| 满载 | 19.9 Hz | 1960 Hz | 80.9° | 43.3 dB |
| 50% | 9.95 Hz | 993 Hz | 84.8° | 49.3 dB |
| 25% | 4.97 Hz | 498 Hz | 85.9° | 55.3 dB |
...
> 4× 变化。随后 LCR 实测原边 Lm=2mH（m=7.4，非当初假设的 m=5），K_f 修正为
> -0.505 V/kHz，K_c 改为 -2.4922e7 保持 fco=1960Hz。本版已统一为标准 FHA 公式
> + 实测 m，固件 2p2z 系数已同步更新。M_max 随 m 修正为 1.005（满载最低输入 398V）。
